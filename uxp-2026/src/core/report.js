export function mediaFileNameForMarker(index, mediaType) {
...
export function normalizeMarkers(markers) {
  return [...markers]
    ...
    .map((marker, offset) => {
      ...
      const hasDuration = Number(marker.durationTicks || 0) > 0;
      const mediaType = hasDuration ? 'video' : 'image';
      return {
        index,
        guid: marker.guid || '',
        name: marker.name || '',
        comment: marker.comment || marker.comments || '',
        startSeconds: Number(marker.startSeconds || 0),
        durationSeconds: Number(marker.durationSeconds || 0),
...
export function buildReportHtml({ sequenceName, generatedAt, markers }) {
  const showMarkerColumn = hasAnyMarkerName(markers);
  const rows = markers.map((marker) => renderMarkerRow(marker, showMarkerColumn)).join('\n');
  const emptyRow = markers.length === 0
    ? '<tr><td colspan="' + (showMarkerColumn ? '6' : '5') + '" class="empty">No sequence markers found.</td></tr>'
    : '';
  const markerHead = showMarkerColumn ? '<th class="col-marker">Marker</th>' : '';
  return '<!doctype html>\n' +
    '<html lang="zh-CN">\n<head>\n' +
    '  <meta charset="utf-8">\n' +
...
function renderMarkerRow(marker, showMarkerColumn) {
  const mediaPath = 'assets/' + marker.assetFileName;
  const resolvedPoster = marker.posterFileName === ''
    ? ''
    : marker.posterFileName || posterFileName(marker.assetFileName);
  const poster = resolvedPoster ? ' poster="assets/' + escapeAttr(resolvedPoster) + '"' : '';
  const media = marker.mediaAvailable === false
    ? '<div class="media-unavailable">Preview unavailable</div>'
    : marker.mediaType === 'video'
    ? '<div class="video-frame"><video src="' + escapeAttr(mediaPath) + '"' + poster + ' preload="metadata"></video><button class="play-overlay" type="button" aria-label="Play preview">▶</button></div>'
    : '<img src="' + escapeAttr(mediaPath) + '" alt="Marker ' + marker.index + ' still">';
  const markerCell = showMarkerColumn ? '<td><div class="marker-name">' + escapeHtml(isRealMarkerName(marker.name) ? marker.name : '') + '</div></td>' : '';
